...
命令面板使用模糊匹配；输入 `>` 只看动作、`@` 只看 JSONL 历史、`:` 只看 workflow、`?` 提交自然语言命令请求。历史和 workflow 只写入当前编辑行；AI 候选先留在审阅界面，所有路径都不会自动按 Enter。所有审阅式插入都拒绝 CR、LF、NUL 和终端控制字符，避免多行条目越过“不提交”边界。`Ctrl+Alt+G` 打开绑定当前 Block pane 的 Shell Agent：严格解析模型 proposal，允许编辑、Reject 或逐条 **Approve & Run**，并把完成块的退出码和截断输出回灌到下一轮。欢迎 Notebook 也可从面板中的 **Open welcome & quick start notebook** 打开。
