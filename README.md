...
- SSH 主机选择、连接复用与自动重连
- 可选多 provider AI，以及绑定当前 Block pane 的原生 Shell Agent；每条候选命令均可编辑且需单独批准
...
AI 面板的分隔条宽度会随配置持久化；完成的聊天回合与实际发送的选中 Block 上下文（默认先脱敏）跟随各自窗口快照恢复，**New chat** 会同步清除。输入框使用 `Enter` 或 `Ctrl+Enter` 发送，`Shift+Enter` 换行，并保留输入法候选确认语义。
