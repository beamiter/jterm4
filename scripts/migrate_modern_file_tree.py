#!/usr/bin/env python3
"""Migrate the sidebar browser from deprecated TreeView/TreeStore to GTK4 list models."""
...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f"expected one match in {path} but found {count}: {old[:80]!r}")
    path.write_text(text.replace(old, new, 1))


FILE_TREE = r'''//! file_tree — asynchronous GTK4 sidebar file browser for UiState.
//!
...
(root / "src/ui/file_tree.rs").write_text(FILE_TREE)

ui_mod = root / "src/ui/mod.rs"
replace_once(
    ui_mod,
    '''#![allow(deprecated)]
...
end = '''fn connect_file_tree_handlers(file_tree: &gtk4::TreeView, ui: &UiState) {
...
if start_index < 0:
    raise SystemExit("could not find deprecated file-tree builder in src/main.rs")
end_index = text.find(end, start_index)
if end_index < 0:
    raise SystemExit("could not find deprecated file-tree handler in src/main.rs")
end_index += len(end)
...
for path in [root / "src/main.rs", root / "src/ui/mod.rs", root / "src/ui/file_tree.rs"]:
    text = path.read_text()
    for stale in ("TreeStore", "TreeView", "file_tree_scan_generation", "file_tree_store"):
        if stale in text:
            raise SystemExit(f"stale {stale!r} remains in {path}")

print("modern GTK4 file-tree migration applied")
