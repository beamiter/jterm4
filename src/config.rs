...
pub struct Config {
    ...
    /// Upper bound on visible rows when a finished block is expanded via its
...
fn load_file_config() -> FileConfig {
    ...
    FileConfig {
        ...
        finished_block_max_expanded_rows: table.get("finished_block_max_expanded_rows").and_then(|v| v.as_integer()).map(|v| v as u32),
...
pub(crate) fn load_config() -> (Config, Vec<Theme>, KeybindingMap) {
    ...
    let finished_block_viewport_rows = env_u32("JTERM4_FINISHED_VIEWPORT_ROWS")
        ...
        .unwrap_or(30)
        .clamp(3, 200);
    let finished_block_max_expanded_rows = env_u32("JTERM4_FINISHED_MAX_EXPANDED_ROWS")
